...
export default function Home() {
  ...
  const difficultyModes: Record<string, { name: string; offensePenalty: number; levelPenalty: number; color: string }> = {
    ...
    legend: { name: '传奇', offensePenalty: 0, levelPenalty: 0, color: 'orange' },
    esports: { name: '电竞', offensePenalty: 0, levelPenalty: 6, color: 'red' }
  };
  ...
  const [difficulty, setDifficulty] = useState<'normal'|'expert'|'master'|'legend'|'esports'>('normal');

  const calculateEarthquakeDamage = () => {
    const basePercentage = earthquakeData[earthquakeLevel - 1].percentage;
    let totalPercentage = 0;
    for (let i = 1; i <= earthquakeCount; i++) totalPercentage += basePercentage / i;
    return totalPercentage;
  };

  const calculateTotalDamage = () => {
    const mode = difficultyModes[difficulty];
    let effectiveFireballLevel = Math.max(1, fireballLevel - mode.levelPenalty);
    const fireball = fireballData[effectiveFireballLevel - 1];
    const fireballDamage = fireball.damage; // 难度不影响伤害
            ...
            {Object.entries(difficultyModes).map(([key, mode]) => (
              <button
                ...
                className={`p-3 rounded-xl font-semibold transition-all border-2 ${
                  difficulty === key ? getActiveColorClasses(mode.color) : getColorClasses(mode.color)
                }`}
              >
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-gray-800">6格</span>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-5 border border-gray-200">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-2">
                  ...
                  <p className="text-xs text-gray-500 pt-2 border-t border-gray-300">💡 不受难度模式影响</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-sm p-5 border border-gray-200">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg text-sm text-blue-700">💡 防御建筑血量不受难度模式影响</div>
              </div>
            </div>

            <div className={`rounded-2xl shadow-lg p-6 border-2 transition-all ${
              result.isDestroyed ? 'bg-gradient-to-br from-red-50 to-orange-50 border-red-400' : 'bg-gradient-to-br from-green-50 to-emerald-50 border-green-400'
            }`}>
              <div className="text-center mb-5">
                <h2 className="text-2xl font-bold text-gray-800">{result.isDestroyed ? '🎯 建筑摧毁！' : '📊 伤害统计'}</h2>
              </div>
                ...
                {!result.isDestroyed && (
                  <div className="bg-white/70 backdrop-blur p-4 rounded-xl border border-gray-200">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3 mt-3">
                      <div className="bg-gradient-to-r from-green-500 to-emerald-500 h-3 rounded-full transition-all duration-500" style={{ width: `${(result.remainingHP / buildingHP) * 100}%` }}></div>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-200">
              ...
              <div className="space-y-2 text-xs text-gray-600">
                <p className="flex items-start gap-2"><span className="text-yellow-600 font-bold">⚡</span><span>地震法术伤害递减：第1个100%效果，第2个50%，第3个33.3%，依此类推</span></p>
                <p className="flex items-start gap-2"><span className="text-orange-600 font-bold">🔥</span><span>难度模式不影响大火球伤害</span></p>
                <p className="flex items-start gap-2"><span className="text-red-600 font-bold">🎮</span><span>电竞模式使装备等级-6级</span></p>
                <p className="flex items-start gap-2"><span className="text-orange-600 font-bold">📏</span><span>大火球范围为 6 格</span></p>
              </div>
            </div>
          </div>
        </div>
